...
<div class="brand"><div class="logo">✉</div><div><h1>AI EMAIL ANALYZER</h1><p>Stable analysis with guaranteed clickable email links</p></div></div>
...
<div class="fallback" id="fallbackNote">AI summary was unavailable, but the email details and clickable links were still extracted successfully.</div>
...
def fallback_report(text,links,lang):
    ...
    count=len(links)
    if lang=="Urdu":
        summary=f"یہ ای میل خودکار طور پر پڑھی گئی ہے۔ اس میں {count} قابلِ کلک متعلقہ لنکس ملے ہیں۔"
        about="ای میل کا مکمل AI خلاصہ دستیاب نہیں ہوا، لیکن اہم معلومات اور تمام دستیاب لنکس محفوظ کر لیے گئے ہیں۔"
    elif lang=="Arabic":
        summary=f"تمت قراءة البريد آلياً وتم العثور على {count} رابطاً قابلاً للنقر."
        about="لم يتوفر ملخص الذكاء الاصطناعي، لكن تم حفظ المعلومات الأساسية والروابط المتاحة."
    else:
        summary=f"The email was processed successfully and {count} relevant clickable link(s) were found."
        about="The AI summary was unavailable, but the app preserved the email details and all available relevant links."
    return {
        "metadata":metadata,"short_summary":summary,"what_email_is_about":about,
        "key_information":[{"label":"Clickable links","value":str(count),"url":""},{"label":"Email length","value":f"{len(text):,} characters","url":""},{"label":"Processing","value":"Completed","url":""},{"label":"Link extraction","value":"Direct from email","url":""}],
        "recommended_actions":[{"title":"Review clickable items","detail":"Open the relevant article, job, product, or offer links below."},{"title":"Verify important details","detail":"Confirm dates, prices, salary, and sender information on the official page."}],
        "ai_fallback":True
    }
